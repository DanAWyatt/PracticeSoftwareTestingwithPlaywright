...
Performance testing is guided by the **ISTQB Advanced Level Technical Test Analyst (CTAL-TTA)** syllabus, with practical automation support from the **ISTQB Specialist Test Automation Skills (CT-TAS)** syllabus.

- **Objective:** Validate that critical user journeys (Search, Product Detail, Add to Cart, Checkout, Login) meet defined response-time and stability targets under realistic load.
- **Test Types:** Baseline, Load, Stress, and Endurance (Soak) testing aligned to business risk and peak shopping behavior.
- **Primary Metrics:** Response Time (P95/P99), Throughput (transactions per second), Error Rate, and UI/API timeout frequency.
- **Approach:** Combine browser-level timing assertions in Playwright with API-level measurements to isolate frontend vs. backend bottlenecks.
- **Environment Controls:** Execute against stable test windows outside hourly purge boundaries where possible; tag and quarantine noisy runs caused by environmental resets.
- **Pass Guidance:** No critical checkout or authentication transaction exceeds agreed latency/error thresholds at nominal load; no progressive degradation in endurance runs.
